...
class YoutubeCommentDownloader:
    ...
    def get_comments_from_url(self, youtube_url, sort_by=SORT_BY_RECENT, language=None, sleep=.1):
        ...
        print(f"{Colorlog.green_color}{datetime.datetime.now()} ---------->>>>>  GET COMMENT OF {youtube_url}{Colorlog.reset_color}")
        ...
        while continuations:
            ...
            video_id = 'yt_'+str(youtube_url).split('watch?v=')[-1]
            for comment in reversed(list(self.search_dict(response, 'commentEntityPayload'))):
                ...
                result = {
                    ...
                    'decription': '',
                    'link': str(youtube_url+"&lc="+properties['commentId'])
                        }
